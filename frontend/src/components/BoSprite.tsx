...
export default function BoSprite({ snapTriggered, onSnapComplete, visible }: Props) {
  ...
  const drawBo = useCallback((timestamp: number) => {
    ...
    const img = isSnapping ? boSnapImg.current : boIdleImg.current;
    const totalFrames = isSnapping ? SNAP_FRAMES : IDLE_FRAMES;
    const fps = isSnapping ? SNAP_FPS : IDLE_FPS;
    const frameRef = isSnapping ? snapFrame : idleFrame;
